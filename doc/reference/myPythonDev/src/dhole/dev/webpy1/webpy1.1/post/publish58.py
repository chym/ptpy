    ...
    def goSubmit(self):
        ...
        if '''<script>parent.window.location.href='http://my.58.com/InfoPressSuccess/?infoid='''in page:
            if re.search('''<script>parent.window.location.href='(.*)'</script></form>''', page):
                ...
                req=urllib2.Request(note, None, self.header)
                fpage=self.br.open(req).read()
                if re.search('''<a href="(.*)">.*</a> 已经成功发布到''', fpage):
                    link=re.search('''<a href="(.*)">.*</a> 已经成功发布到''', fpage).group(1)
                    return "success|%s"%(link,)
        else:
...
if __name__=="__main__":
    ...
    "house_floor":"2",
    "house_age":"10",
    "house_desc":'''<SPAN id=comp-paste-div-700><font color='red'>实际未来城三期实际未来城三期实际未来城三期实际未来城三期</font></SPAN>''',
    "borough_id":"10",
    "borough_name":"实际未来城三期实际未来城三期实际未来城三期",
