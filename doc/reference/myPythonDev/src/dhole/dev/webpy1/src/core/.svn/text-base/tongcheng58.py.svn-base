...
import random
from common.config import housetype, checkPath, makePath,fitment,toward,deposit
import threading
from BeautifulSoup import BeautifulSoup
homepath="d:\\home\\spider\\"
class LinkCrawl(object):
    def __init__(self,citycode="",kind="",st=3):
...
class ContentCrawl(object):
    def __init__(self,links,citycode,kind,st):
        cj = cookielib.MozillaCookieJar()
        self.br=urllib2.build_opener(urllib2.HTTPHandler(),urllib2.HTTPCookieProcessor(cj),urllib2.HTTPRedirectHandler())
        self.pdb={}
        self.header={
    ...
    def extractDict(self):    
        for url in self.urls:
            if checkPath(homepath,self.folder,url):
                ...
                #超过七天
#                if (time.time() -self.fd["posttime"]) > 7*24*3600:return None
...
class fetchData(threading.Thread): 
    def __init__(self,d):
        ...
        self.d=d
    def run(self):
        lc=LinkCrawl(self.d["citycode"],self.d["kind"],self.d["st"])
        clinks=lc.runme()
        cc=ContentCrawl(clinks,self.d["citycode"],self.d["kind"],self.d["st"])
        cc.extractDict()
def getLinks(d):
    lc=LinkCrawl(d["citycode"],d["kind"],d["st"])
    clinks=lc.runme()
    return clinks
def getContent(clinks,d):
    cc=ContentCrawl(clinks,d["citycode"],d["kind"],d["st"])
    fd=cc.extractDict()
    return fd
...
#    lc.runme()
    ...
    cc=ContentCrawl(["http://su.58.com/ershoufang/4796825138817x.shtml"],citycode="su",kind="1",st=1)
    cc.extractDict()