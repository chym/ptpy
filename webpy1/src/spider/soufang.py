...
#        self.fd['info_type'] = '' 
    ...
    def extractDict(self):
        ...
        if checkPath(homepath,self.folder,self.urls):
            ...
            if self.kind=="3":
                self.QiuGou(self.urls)
            elif self.kind=="4":
                self.QiuZu(self.urls)
            elif self.kind=="2":
                self.ChuZhu(self.urls)
            elif self.kind=="1":
                self.ChuShou(self.urls)
...
#    lc.runme()
    ...
    cc=ContentCrawl("http://esf.suzhou.soufun.com/chushou/1_119958101_-1.htm",citycode="suzhou",kind="1")
    cc.extractDict()
