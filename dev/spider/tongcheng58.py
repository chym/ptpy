...
class ContentCrawl(object):
    def __init__(self,links,citycode,kind,upc):
        ...
        self.house_addr2_regex="<li><i>地址：</i>(.*?)</a>"
        self.house_region_regex="<li><i>区域：</i>(.*?)</li>"
...
#    cc.extractDict()
    import gc
    gc.enable()
#    s=getLinksThread({"citycode":"liaoyang","kind":"4"})
#    s.start()
#    lc=LinkCrawl(citycode="liaoyang",kind="4")
#    lc.runme()
#    print gc.collect(),len(gc.garbage),len(gc.get_objects())

    cc=ContentCrawl("http://su.58.com/ershoufang/7218456118664x.shtml",citycode="su",kind="3",upc="50")
    cc.extractDict()
